                ...
                <td>
                    <select name="barangay_id" id="" class="form-control" required>
                        ...
                        @foreach($user_brgy as $brgy)
                            <option value="{{ $brgy->barangay_id }}" <?php if(isset($sitio->sitio_barangay_id)){if($sitio->sitio_barangay_id==$brgy->barangay_id) echo 'slected';} ?> >{{ \App\Barangay::find($brgy->barangay_id)->description }}</option>
                        @endforeach
                    </select>
                </td>
            </tr>
