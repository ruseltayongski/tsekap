...
class RiskProfileController extends Controller
{
    ...
    private function getUserFacilityMapping($user){
        $userHealthFacilityMapping = UserHealthFacility::where('user_id', $user->id)->first();
        return $userHealthFacilityMapping->facilty_id;
    }
    ...
    public function SubmitRiskPForm(Request $request)
    {
        ...
        $existingRiskProfile = RiskProfile::where('profile_id', $request->profile_id)
            ...
            ->where('facility_id_updated', $this->getUserFacilityMapping($user))
            ->get();

        \Log::info('Existing Risk Profile: ' . json_encode($existingRiskProfile));

        if ($existingRiskProfile->count() > 1) {
            // Redirect with an error message if more than one duplicate is found
        ...
        $riskform->pmh_heart_disease = $request->pmh_heart_disease;
        $riskform->pmh_diabetes = $request->pmh_diabetes;
        $riskform->pmh_specify_diabetes = $request->pmh_diabetes_details;
        $riskform->pmh_cancer = $request->pmh_cancer;
        $riskform->pmh_specify_cancer = $request->pmh_cancer_details;
        $riskform->pmh_copd = $request->pmh_COPD;
        $riskform->pmh_asthma = $request->pmh_asthma;
        $riskform->pmh_allergies = $request->pmh_allergies;
        $riskform->pmh_specify_allergies = $request->pmh_allergies_details;
        $riskform->pmh_mn_and_s_disorder = $request->pmh_mnsad;
        $riskform->pmh_specify_mn_and_s_disorder = $request->pmh_mnsad_details;
        $riskform->pmh_vision_problems = $request->pmh_vision;
        $riskform->pmh_previous_surgical = $request->pmh_psh;
        $riskform->pmh_specify_previous_surgical = $request->pmh_psh_details;
        $riskform->pmh_thyroid_disorders = $request->pmh_thyroid;
        $riskform->pmh_kidney_disorders = $request->pmh_kidney;

        //FAMILY HISTORY
        $riskform->fmh_hypertension = $request->fmh_hypertension;
        $riskform->fmh_stroke = $request->fmh_stroke;
        $riskform->fmh_heart_disease = $request->fmh_heart_disease;
        $riskform->fmh_diabetes_mellitus = $request->fmh_diabetes_mellitus;
        $riskform->fmh_asthma = $request->fmh_asthma;
        $riskform->fmh_cancer = $request->fmh_cancer;
        $riskform->fmh_kidney_disease = $request->fmh_kidney;
        $riskform->fmh_first_degree_relative = $request->fmh_first_degree;
        $riskform->fmh_having_tuberculosis_5_years = $request->fmh_famtb;
        $riskform->fmh_mn_and_s_disorder = $request->fmh_mnsad;
        $riskform->fmh_copd = $request->fmh_copd;

        // NCD RISK FACTORS 
        $tobaccoUsed = $request->tobaccoUse;
        $tobaccoUseLimited = array_slice($tobaccoUsed, 0, 2);
        $riskform->rf_tobacco_use = implode(', ', $tobaccoUseLimited);
        $riskform->rf_alcohol_intake = $request->ncd_alcohol;
        $riskform->rf_alcohol_binge_drinker = $request->ncd_alcohol_binge;
        ...
        $riskform->rs_systolic_t1 = $request->systolic_t1;
        $riskform->rs_diastolic_t1 = $request->diastolic_t1;
        $riskform->rs_systolic_t2 = $request->systolic_t2;
        $riskform->rs_diastolic_t2 = $request->diastolic_t2;
        $riskform->rs_blood_sugar_fbs = $request->fbs_result;
        $riskform->rs_blood_sugar_rbs = $request->rbs_result;
        $riskform->rs_blood_sugar_date_taken = $request->blood_sugar_date_taken;
        $riskform->rs_blood_sugar_symptoms = implode(', ', $dmSymptoms);
        $riskform->rs_lipid_cholesterol = $request->lipid_cholesterol;
        $riskform->rs_lipid_hdl = $request->lipid_hdl;
        ...
        $riskform->rs_urine_ketones_date_taken = $request->uri_ketones_date_taken;
        ...
        $riskform->mngm_med_hypertension = $request->anti_hypertensives;
        $riskform->mngm_med_hypertension_specify = $request->anti_hypertensives_specify;

        // Anti-Diabetes: Store selected option and any specify text, and type
        $riskform->mngm_med_diabetes = $request->anti_diabetes;
        $riskform->mngm_med_diabetes_options = $request->anti_diabetes_type;
        $riskform->mngm_med_diabetes_specify = $request->anti_diabetes_specify;

        // Follow-up Date
        $riskform->mngm_date_follow_up = $request->follow_up_date;

        // Remarks (Text area)
        $riskform->mngm_remarks = $request->remarks;
